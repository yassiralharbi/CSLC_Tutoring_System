...
$postback[]="Please use pdf and less than 2 MB";


if(!empty($error)){
    header('Location:TutorSignUp.php?postback='.Join($postback, urlencode(',') ));
    exit;
}
...
$sql = " insert into tutors (tutor_Id,FirstName,LastName,email,phone,get_hash)VALUES('$tutor_Id','$FirstName','$LastName','$email','$phone','$get_hash');";
mysql_query($sql);

$sql = " insert into logins (ID,type)VALUES('$tutor_Id','tutor');";
mysql_query($sql);
...
if(mysql_affected_rows($connect) == 1)
{
    ...
    echo "<p1>Thank you for your application, $FirstName.<br/>The administrator will be in contact with you shortly.</p1>";
}
else
{
    header("Refresh: 3; URL='../index.php'");
    echo "<p1>Sorry $FirstName, your application could not be processed. Please contact the administrator to report an issue.</p1>";
}
